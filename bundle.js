...
var app = (function () {
    ...
    /* src/Expandable.svelte generated by Svelte v3.6.7 */

    const file = "src/Expandable.svelte";

    function create_fragment(ctx) {
    	var div, t, div_transition, current, dispose;

    	return {
    		c: function create() {
    			div = element("div");
    			t = text(ctx.text);
    			add_location(div, file, 26, 0, 452);
    			dispose = listen(div, "click", ctx.toggleVisibility);
    		},
    ...
    let more = "<div class=sup>+</div>";

    function instance($$self, $$props, $$invalidate) {
    	let { input, visible } = $$props;
        let text;
        function toggleVisibility() {
            $$invalidate('visible', visible = !visible);
            if (visible) {
                $$invalidate('text', text = more);
            } else {
                $$invalidate('text', text = input);
            }
        }

    	const writable_props = ['input', 'visible'];
    	Object.keys($$props).forEach(key => {
    		if (!writable_props.includes(key) && !key.startsWith('$$')) console.warn(`<Expandable> was created with unknown prop '${key}'`);
    	});
    	...
    	return { input, visible, text, toggleVisibility };
    }

    class Expandable extends SvelteComponentDev {
    	constructor(options) {
    		super(options);
    		init(this, options, instance, create_fragment, safe_not_equal, ["input", "visible"]);

    		const { ctx } = this.$$;
    		const props = options.props || {};
    		if (ctx.input === undefined && !('input' in props)) {
    			console.warn("<Expandable> was created without expected prop 'input'");
    		}
    		if (ctx.visible === undefined && !('visible' in props)) {
    			console.warn("<Expandable> was created without expected prop 'visible'");
    		}
    	}
    ...
    function create_fragment$1(ctx) {
    	...
    	var expandable = new Expandable({
    		props: {
    		input: "Some text that can be hidden or shown",
    		visible: "true"
    	},
    		$$inline: true
    	});
    	...
    	return {
    		c: function create() {
    			...
    			t2 = text("!");
    			t3 = space();
    			expandable.$$.fragment.c();
    			attr(h1, "class", "svelte-i7qo5m");
    			add_location(h1, file$1, 11, 0, 132);
    		},
